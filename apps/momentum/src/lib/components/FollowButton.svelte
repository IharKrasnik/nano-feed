<script>
	...
	let sortProjectsByFollowing = () => {
		$allProjects = [
			...$allProjects.sort(
				(a, b) => Number(b.isFollowing || false) - Number(a.isFollowing || false)
			)
		];
		return $allProjects;
	};
    ...
    {/if} -->
		...
		{#if project && project.slug && project.creator?._id !== $currentUser._id}
			{#if $allProjects.find((f) => f._id === project._id).isFollowing}
				{#if !isOnlyFollow}
					<div
						class="w-full font-bold text-sm cursor-pointer hover:underline text-center"
						on:click={unfollowStream}
					>
						{#if isShort}
							✓ Following
						{:else}
