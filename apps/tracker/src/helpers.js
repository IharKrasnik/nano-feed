    ...
    if (id) {
      ...
      if (parentNode) {
        ...
        if (sibling.length > 1 && countSameNodeName > 1) {
          const classNames = elementItem.className ? `.${elementItem.className.replace(CLASS_DIVIDER, '.')}` : '';
...
export const sendData = (url, data) => fetch(`${API_URL}/${url}`, {
  method: 'POST',
  headers: {
...
export const sendTrackData = (userData, url, data) => sendData(
  url,
  {
    ...data,
...
export const throttle = (func, limit) => {
  ...
  return (args) => {
    ...
    isBlocked = true;
    setTimeout(() => { isBlocked = false; }, limit);
  };
};
