...
const calcIdentificator = () => new Promise((res, rej) => {
...
const init = (initialUserData, handlers = defaultHandlers) => () => {
    ...
    .then((visitorId) => {
      ...
      return fetch('https://api.ipify.org')
        ...
        .then((data) => {
          ...
          const session = JSON.parse(localStorage.getItem('waveloop_session'));
          ...
          return sendData('init', { visitorId, projectId: userData.projectId, payload });
        });
    }).then(({ sessionId }) => {
      if (sessionId) {
        const setSession = () => {
